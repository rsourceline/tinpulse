...
────────────────────────────────────────────────────────────────────────────
Nightly fundamentals & link updater for TinPulse.

Adds / refreshes for “core” coins (rank ≤ 1 250  OR 24 h vol ≥ $5 M):
...
Usage
    python update_daily_meta.py --limit 800 -v
"""

from pathlib import Path
import sys, time, json, os, requests, pandas as pd

# ─── CLI flags ────────────────────────────────────────────────────────────
MAX_COINS = int(sys.argv[sys.argv.index("--limit")+1]) if "--limit" in sys.argv else 500
VERBOSE   = "-v" in sys.argv or "--verbose" in sys.argv

# ─── CONFIG ───────────────────────────────────────────────────────────────
CSV_PATH       = Path("cryptos.csv")
LU_CACHE_PATH  = Path("last_updated_cache.json")   # coin_id -> last_updated ISO
DEFER_CACHE    = Path("daily_defer.json")          # coin_id -> unix_ts
...
MARKETS_SLEEP   = 0.3   # delay between /markets pages
COIN_SLEEP      = 1.0   # delay between /coins/{id} calls
MAX_429_RETRIES = 2
DEFER_HOURS     = 24

CORE_RANK_MAX = 1250
CORE_VOL_MIN  = 5_000_000   # USD
# ──────────────────────────────────────────────────────────────────────────
...
def deep_get(d, *path):
    for p in path:
        if isinstance(p, int):
            if not isinstance(d, list) or len(d) <= p:
                return ""
            d = d[p]
        else:
            d = d.get(p, {})
...
def fetch_markets():
    """Return rank_map, vol_map, ts_map (with 429 back-off)."""
    ...
    while True:
        ...
        if r.status_code == 429:
            ...
            wait = 20 * retries
            if VERBOSE:
                print(f"  /markets 429 page {page} – wait {wait}s (retry {retries})")
            time.sleep(wait)
            continue
        ...
        if not isinstance(chunk, list):
            raise RuntimeError("/markets non-JSON body")
...
def is_core(cid, rank_map, vol_map):
    return (rank_map.get(cid, 999_999) <= CORE_RANK_MAX or
            vol_map.get(cid, 0) >= CORE_VOL_MIN)

def fetch_meta(cid):
    """Return dict of fields or {} if skipped/deferred."""
    ...
    while True:
        ...
        try:
            ...
            if VERBOSE: print(f"  Non-JSON body {cid} – defer")
            defer[cid] = time.time(); return {}

        pct_30d = deep_get(j, "market_data",
                           "price_change_percentage_30d_in_currency", "usd")
        plats   = j.get("platforms") if isinstance(j.get("platforms"), dict) else {}
        chains  = "|".join(plats.keys())
        contract= plats.get("ethereum", "")
        ...
        return dict(
            ...
            telegram_url      = (
                f"https://t.me/{deep_get(j,'links','telegram_channel_identifier')}"
            ...
            twitter_url       = (
                f"https://twitter.com/{deep_get(j,'links','twitter_screen_name')}"
...
def main():
    ...
    if not candidates:
        print("Daily meta: up-to-date.")
        return

    print(f"Processing {len(candidates)} coin(s)…")
    updates, start = [], time.time()
